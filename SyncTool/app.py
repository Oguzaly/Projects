...
cur=conn.cursor()
cur.execute("select guid,content_status,type from atlas_cms_vod.content where (type='MOVIE' or type='EPISODE') and content_status='WaitingForMaterials'")
frsh =cur.fetchall()
with open('C:/Users/HP/Desktop/playground/SyncTool/status.csv','a') as f:
    for i in frsh:
        ...
        if i[2]=='MOVIE':
            ...
            print(response.text)
            type=str(i[0])+','+str(i[1])+','+str(i[2])+','+ response.text
            f.write(type+'\n')
        else :
